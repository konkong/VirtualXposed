...
import io.virtualapp.widgets.filepicker.utils.StringUtils;
...
public class FilePickerActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        updateAddButton();
        if (!checkSDState()) {
            Toast.makeText(this, R.string.file_picker_NotFoundPath, Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        mFilter = new FilePickerFilter(mParamEntity.getFileTypes());
        mListFiles = FileUtils.getFileList(mPath, mFilter);
...
//                mToolbar.setNavigationIcon(R.mipmap.file_picker_back1);
//                break;
//            case Constant.BACKICON_STYLETWO:
//                mToolbar.setNavigationIcon(R.mipmap.file_picker_back2);
//                break;
...
//        }
    ...
    private void updateAddButton() {
        ...
        if (!mParamEntity.isChooseMode()) {
            ...
            if (mParamEntity.getAddText() != null) {
                ...
                mBtnAddBook.setText(getString(R.string.file_picker_OK));
            }
            //文件夹模式默认为单选模式
    ...
    private void initListener() {
        ...
        mTvBack.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                updateMenuTitle();
                mBtnAddBook.setText(getString(R.string.file_picker_Selected));
                ...
                if (mParamEntity.getAddText() != null) {
                    ...
                    mBtnAddBook.setText(R.string.file_picker_Selected);
                }
            }
        });
                ...
                if (mParamEntity.isMutilyMode()) {
                    ...
                    if (mListFiles.get(position).isDirectory()) {
                        ...
                        if (mParamEntity.getAddText() != null) {
                            ...
                            mBtnAddBook.setText(getString(R.string.file_picker_Selected) + "( " + mListNumbers.size() + " )");
                        }
                        //先判断是否达到最大数量，如果数量达到上限提示，否则继续添加
                        if (mParamEntity.getMaxNum() > 0 && mListNumbers.size() > mParamEntity.getMaxNum()) {
                            Toast.makeText(FilePickerActivity.this, R.string.file_picker_OutSize, Toast.LENGTH_SHORT).show();
                            return;
                        }
                    } }catch (Exception ex){ VLog.e(TAG, ex.getMessage());}
                } else {
                    //单选模式直接返回
                    if (mListFiles.get(position).isDirectory()) {
                        chekInDirectory(position);
                    ...
                    if (mParamEntity.isChooseMode()) {
                        ...
                        Toast.makeText(FilePickerActivity.this, R.string.file_picker_ChooseTip, Toast.LENGTH_SHORT).show();
                    }
                }
            ...
            public void onClick(View v) {
                if (mParamEntity.isChooseMode() && mListNumbers.size() < 1) {
                    ...
                    if (TextUtils.isEmpty(info)) {
                        Toast.makeText(FilePickerActivity.this, R.string.file_picker_NotFoundBooks, Toast.LENGTH_SHORT).show();
                    } else {
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        getMenuInflater().inflate(R.menu.menu_file_picker_toolbar, menu);
        this.mMenu = menu;
    ...
    public void updateMenuTitle() {
        ...
        if (mIsAllSelected) {
            mMenu.getItem(0).setTitle(getString(R.string.file_picker_Cancel));
        } else {
            mMenu.getItem(0).setTitle(getString(R.string.file_picker_SelectAll));
        }
    }
